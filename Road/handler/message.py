...
class MessageUserUnreadCountHandler(BaseHandler):
    def get(self):
        user_info = self.get_current_user()
        strSql = "select count(*) count from message_info where rcv_id = "+user_info["id"]+" and TO_DAYS(now()) - TO_DAYS(post_time) <= 30 and status = 0"
...
class MessageListHandler(BaseHandler):
    def get(self):
        ...
        user_info = self.get_current_user()
        strSql = "select * from message_info where rcv_id = %s" % user_info["id"]
